...
export const makingReqforNSE =
  ...
  (dispatch) => {
    ...
    axios
      .get(`http://192.168.1.7/NSE/GetAllNSEDataBySym?interval=-15&symbol=${symbol}`)
      .then((res) => {
        console.log(res);
...
export const makingReqforTimeIntData = (interval, symbol) => (dispatch) => {
  ...
  axios
    .get(`http://192.168.1.7/NSE/GetAllNSEDataBySym?interval=${interval}&symbol=${symbol}`)
    .then((res) => {
      console.log(res);
    })
...
export const getNIFTYExpiryDate = (symbol) => (dispatch) => {
  axios
    .get(`http://192.168.1.7/NSE/GetAllExpiries?symbol=${symbol}`)
    .then((res) => {
