...
const OpenIntrestes = () => {
  ...
  let twoMonthData = useSelector((store) => store.reducer.twoMonthData);
    ...
    if (fiveMinBool) {
      ...
      const data = firstFiveFromTwoMonth.map(
        (option, index) =>
          ...
          (firstFiveFromFivemin[index].combinedCEPE.CE_openInterest || 0)
      );
      console.log(data);
      return data;
    } else {
      return firstFiveFromTwoMonth.map((option) => option.combinedCEPE.CE_openInterest || 0);
    }
  };
  ...
  const PEtimeBasedData = () => {
    if (fiveMinBool) {
      const data = firstFiveFromTwoMonth.map(
        (option, index) =>
          (fiveMinData[index].combinedCEPE?.PE_openInterest || 0) -
          (option.combinedCEPE.PE_openInterest || 0)
      );
      console.log(data);
      return data;
    } else {
      return firstFiveFromTwoMonth.map((option) => option.combinedCEPE.PE_openInterest || 0);
    }
  };
